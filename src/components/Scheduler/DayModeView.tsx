...
export type Column = { weekDay?: number; month?: number; day?: number; headerName?: string; date?: Date };
export type Day = {
    id: string;
    data: Item[];
    date: Date;
...
export type ItemTransfert = {
    item: Item;
    rowLabel: string;
    rowIndex: number;
    dayIndex: number;
};
export type TransfertTarget = {
    item?: Item;
    rowLabel: string;
    rowIndex: number;
    dayIndex: number;
...
interface Props {
    ...
    onCellClick?: (event: React.MouseEvent, row: Row, day: number | undefined) => void;
    onEventsChange?: (item: Item) => void;
}

function DayModeView(props: Props) {
    ...
    const handleCellClick = (event: React.MouseEvent, row: Row, day: number | undefined) => {
                ...
                <TableHead sx={{ height: 24 }}>
                    <StyledTableRow>
                        ...
                        {columns?.map((column, index) => (
                            <StyledTableCell align="center" colSpan={2} key={`weekday-${column?.day}-${index}`}>
                                ...
                                {row?.days?.map((day, dayIndex) => {
                                    return (
                                        <StyledTableCell
                                            ...
                                            onClick={event =>
                                                // @ts-ignore
                                                handleCellClick(event, { rowIndex, ...row }, { dayIndex, ...day })
                                            }
                                        >
                                            {day?.data?.length ||
                                                (0 > 0 && renderTask(day.data || [], row.label, rowIndex, dayIndex))}
                                        </StyledTableCell>
                                    );
                                })}
                            </StyledTableRow>
